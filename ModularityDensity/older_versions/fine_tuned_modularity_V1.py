...
def split_communities_q(adj, c, split_track, merge_track, r, normalize,
    ...
    comm_metric = np.array([modularity_r(adj, c, np.array([cluster_num]),
    ...
    for cluster_num in unique_clusters:
        ...
        if (((sub_adj).sum() == 0) | (not(nx.is_connected(g))) |
           (split_change[cluster_num] != 1)):
            ...
            #Rearrange the nodes of sub_adj in the descreasing order of elements of fieldler vector
            nodeIds = [i for f_vector,i in sorted(zip(f_vector, sub_index), reverse = True)]

            #Initialize the communities corresponding to bipartitioning of 'cluster_num'
            first_community = []
            second_community = []
            ...
            for j in range(len(nodeIds)-1):
                ...
                if(nx.is_connected(g1) & nx.is_connected(g2)):
                    ...
                    #Tracks the nodes in each of the split communities of 'cluster_num'
                    dict_bool_copy = dict()
                    dict_bool_copy[cluster_num] = (c_latest == cluster_num)
                    dict_bool_copy[new_label] = (c_latest == new_label)

                    #Calculate the difference in modularity for splitting 'cluster_num'
                    div_metric = (modularity_r(adj, c_latest, np.unique(c_sub[0:]), r, dict_bool_copy) - curr_metric)
            ...
            #Check if atleast one instance of splitting 'cluster_num' exists that improves modularity
            if len(split_info) > 0:
                #Split 'cluster_num' based on the division that best improves modularity
                best_split = split_info[max(split_info.keys())]
                c_sub[nodeIds[0:best_split+1]] = cluster_num
                new_label = max(c_new) + 1
                c_sub[nodeIds[best_split+1:]] = new_label

                #Update 'c_new' with new community labels as a result of splitting 'cluster_num'
                c_new[bool_r] = c_sub

                #Update the dictionary key-value pair, as the community 'cluster_num' split into two communities
                split_change[cluster_num] = 1
                split_change[new_label] = 1
                merge_change[cluster_num] = 1
                merge_change[new_label] = 1
            else:
                #Set the dictionary value to 0 for the key 'cluster_num' that did not split
                split_change[cluster_num] = 0

    #Resultant integer array of community labels of the nodes (as a result of splitting), updated 'split_change' and updated 'merge_change'
...
def merge_communities_q(adj, c, split_track, merge_track, r):
    ...
    split_track : dictionary
        Tracks the communities fit for splitting; contains cluster labels as dictionary keys, and
        the corresponding binary values (0 or 1) as values; 1 indicates the community is fit for splitting
        0 indicates the community is not fit for splitting.
    merge_track : dictionary
        Tracks the communities fit for merging; contains cluster labels as dictionary keys, and
        the corresponding binary values (0 or 1) as values; 1 indicates the community is fit for merging
        0 indicates the community is not fit for merging.
    r : float
        Resolution of the topology: smaller 'r' favors forming larger communities, while larger 'r' favors forming smaller communities.

    Returns
    -------
    tuple
        Tuple of the array of community labels of the nodes (as a result of merging), updated 'split_track' and 'merge_track'.
    ...
    for comm in unique_clusters:
        ...
        #Check if the community 'comm' is connected to atleast one other community
    ...
    #Determine the contribution of each community to modularity
    comm_metric = np.array([modularity_r(adj, c, np.array([cluster_num]), r, dict_bool) for cluster_num in unique_clusters2])
    ...
    for comm1 in unique_clusters2[:-1]:
        ...
        for comm2 in unique_clusters2[i+1:]:
            ...
            #Consider merging only if 'comm2' is connected to 'comm1', and atleast one of 'comm1' or 'comm2' is fit for merging
            if (((adj_comm1.dot(zero)).sum() != 0) & ((merge_track[comm1] == 1) | (merge_track[comm2] == 1))):
                ...
                #Update the array of community labels to determine the new value of the Modularity (as a result of merging)
                ...
                #Calculate the difference in modularity for merging 'comm1' and 'comm2'
                div_metric = modularity_r(adj, c_latest, np.array([merged_label]), r, dict_bool_copy) - (metric_1 + metric_2)
    ...
    if (len(merging_info) > 0):
        ...
        for div_metric in sorted(merging_info.keys(), reverse = True):
            ...
            #Consider each pair of clusters, which improve modularity when merged
            ...
            if ((not(comm1 in comms_list)) & (not(comm2 in comms_list))):
                ...
                #Update the dictionary key-value pair, as the clusters 'comm1' and 'comm2' have merged to form a new community
    ...
    #Set the dictionary values to 0 for all the keys of the clusters that have not merged
    for cluster in np.unique(c_new):
        if(cluster not in comms_list):
            merge_change[cluster] = 0

    #Resultant integer array of community labels of the nodes (as a result of merging), updated 'split_change' and updated 'merge_change'
    return (c_new, split_change, merge_change)


def fine_tuned_clustering_q(adj, r=0, normalize = True, evd_method ='lanczos', tolerence = 1e-08):
    """Find communities in graph using fine-tuned, modularity maximization. This method supports
    ...
    -------
    Integer array
        Array of community labels for the nodes in the graph as ordered by the adjacency matrix 'adj'.

    """

    #Graph constructed from the adjacency matrix 'adj'
    ...
    for gr in nx.connected_components(G):
        ...
        #Initialize the number of communities formed by splitting the clusters in 'gr'
        split_size = 0

        #Tracks the communities fit for splitting, with cluster labels as dictionary keys, and
        #the corresponding binary values (0 or 1) as values; 1 indicates the community is fit for splitting
        #0 indicates the community is not fit for splitting
        split_track = dict({0: 1})

        #Initialize the number of communities formed by merging the clusters in 'gr'
        merge_size = 0

        #Tracks the communities fit for merging, with cluster labels as dictionary keys, and
        #the corresponding binary values (0 or 1) as values; 1 indicates the community is fit for merging
        #0 indicates the community is not fit for merging
        merge_track = dict({0: 1})

        #Adjacency matrix of 'gr'
        adj_gr = adj[nodes_gr].T[nodes_gr]

        #Iteratively carrying out splitting and merging, alternatively, until neither splitting nor merging
        #of the community structure of 'gr' improves modularity. No improvement in modularity is indicated
        #if the community structure does not change in one full iteration of splitting and merging
        while ((comm_size != split_size) | (comm_size != merge_size)):
            ...
            #Tuple of the array of community labels (of the nodes in 'gr') after splitting, updated 'split_track' and 'merge_track'
            (c_new, split_track, merge_track) = split_communities_q(adj_gr, c_new, split_track, merge_track, r, normalize, evd_method, tolerence)
            split_size = len(np.unique(c_new))

            #Tuple of the array of community labels (of the nodes in 'gr') after merging, updated 'split_track' and 'merge_track'
            (c_new, split_track, merge_track) = merge_communities_q(adj_gr, c_new, split_track, merge_track, r)
            merge_size = len(np.unique(c_new))

        #Update the community labels of the nodes corresponding to 'gr' in original graph 'G'
        c_total[nodes_gr] = np.max(c_total) + 1 + c_new

    #Resultant list of community labels for the nodes in the graph 'G' as ordered by the adjacency matrix 'adj'
    return c_total
