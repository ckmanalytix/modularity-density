        ...
        if (((sub_adj).sum() == 0) | (not(nx.is_connected(g))) |
           (split_change[cluster_num] != 1)):
            ...
            f_vector = fiedler_vector(g, weight='weight', normalized=normalize,
                                      tol=tolerence, method=evd_method)
...
def fine_tuned_clustering_q(adj, r=0, normalize=True, evd_method='lanczos',
                            tolerence=1e-08):
    """Find communities in graph using fine-tuned, modularity maximization.
        ...
        while ((comm_size != split_size) | (comm_size != merge_size)):
            ...
            (c_new, split_track, merge_track) = \
                split_communities_q(adj_gr, c_new, split_track, merge_track,
                                    r, normalize, evd_method, tolerence)
