...
def split_communities_qds(adj, c, normalize, evd_method, tolerence):
    """Splits the communities in graph if the splitting improves modularity density.
    ...
    -------
    Integer array
        Array of community labels, as a result of splitting, for the nodes in the graph as ordered by the adjacency matrix.
    ...
    for cluster_num in unique_clusters:
        ...
        #Don't consider further splitting singleton communities or a community which has disconnected modules
        if (((sub_adj).sum() == 0) | (not(nx.is_connected(g)))):
            if(not(nx.is_connected(g))):
                print("Warning: Check your data as an earliar iteration resulted in a cluster with internal disconnected components")
            continue
        else:

            #Create an array of community labels for the nodes in 'cluster_num'
            ...
            for j in range(len(nodeIds)-1):
                ...
                if(nx.is_connected(g1) & nx.is_connected(g2)):
                    ...
                    #Array of the union of connected clusters of the split communities of 'cluster_num'
                    ...
                    #Update the boolean array of the split communities of 'cluster_num'
                    dict_bool_copy[cluster_num] = (c_latest == cluster_num)
                    dict_bool_copy[new_label] = (c_latest == new_label)

                    #Calculate the difference in modularity density for splitting 'cluster_num'
                    div_metric = (modularity_density(adj, c_latest, np.unique(c_sub[0:]), dict_bool_copy, conn_clusters) - curr_metric)

                    #Record the split only if it improves the modularity density
                    if div_metric > 0:
            ...
            #Check if atleast one instance of splitting 'cluster_num' exists that improves modularity density
            if len(split_info) > 0:
                #Split 'cluster_num' based on the division that best improves modularity density
...
def merge_communities_qds(adj, c):
    ...
    for comm1 in unique_clusters2[:-1]:
        ...
        for comm2 in unique_clusters2[i+1:]:
            ...
            if (comm2 in dict_connected[comm1]):
                ...
                #Update the array of community labels to determine the new value of the Modularity density (as a result of merging)
                ...
                #Calculate the difference in modularity density for merging 'comm1' and 'comm2'
    ...
    #Check if atleast one instance of merging exists that improves modularity density
...
def fine_tuned_clustering_qds(adj, normalize = True, evd_method ='lanczos', tolerence = 1e-08):
    ...
    for gr in nx.connected_components(G):
        ...
        #of the community structure of 'gr' improves modularity density. No improvement in modularity density
        #is indicated if the community structure does not change in one full iteration of splitting and merging
        while ((comm_size != split_size) | (comm_size != merge_size)):
            ...
            #Integer array of community labels of the nodes in 'gr' after splitting
            ...
            #Integer array of community labels of the nodes in 'gr' after merging
