...
def split_communities_qds(adj, c, normalize, evd_method, tolerence):
    ...
    comm_metric = np.array([modularity_density(adj, c,
                            np.array([cluster_num]), dict_bool,
...
def fine_tuned_clustering_qds(adj, normalize=True,
                              evd_method='lanczos', tolerence=1e-08):
    """Find communities in graph using
    ...
    The fine-tuned algorithm iteratively carries out
